 ...
 * @subpackage octabs 
 *
 * 		is a modified fntabs course format
...
if ($editing) {
    $strweekhide = get_string('hideweekfromothers', 'format_octabs');
    $strweekshow = get_string('showweekfromothers', 'format_octabs');
...
$isadmin = has_capability('moodle/course:update', get_context_instance(CONTEXT_COURSE, $course->id)); // added by oncampus
if (($isadmin and $PAGE->user_is_editing()) or (!empty($course->showsection0) && ($thissection->summary or $thissection->sequence or $PAGE->user_is_editing()))) {
// oncampus if (!empty($course->showsection0) && ($thissection->summary or $thissection->sequence or $PAGE->user_is_editing())) {
    ...
    if ($PAGE->user_is_editing()) {
        ...
        if (empty($_GET['edittitle']) or ($_GET['edittitle'] != 'sec0')) {
            ...
            if ($course->sec0title) {
				...
				echo $OUTPUT->heading($course->sec0title, 2, 'sectionzerotextalignment'); // oncampus
            } else {
                //echo $OUTPUT->heading(get_string('sectionzerodefaultheading', 'format_octabs'), 2, 'fnoutlineheadingblock1');
				echo $OUTPUT->heading(get_string('sectionzerodefaultheading', 'format_octabs'), 2); // oncampus
            }
            ...
            echo ' <a title="' . get_string('edit_title_for_section0', 'format_octabs') . '" href="' . $CFG->wwwroot . '/course/view.php?id=' .
            $course->id . '&amp;edittitle=sec0"><img src="' . $pixpath . '/t/edit.gif" /></a>';

        } else if ($_GET['edittitle'] == 'sec0') {
            echo '<form name="editsec0title" method="post" ' .
            'action="' . $CFG->wwwroot . '/course/format/octabs/mod.php">' .
            ...
            if ($course->sec0title) {
                // oncampus echo $OUTPUT->heading($course->sec0title, 2, 'fnoutlineheadingblock1');
            } else {
                // oncampus echo $OUTPUT->heading(get_string('sectionzerodefaultheading', 'format_octabs'), 2, 'fnoutlineheadingblock1');
            }
        }
    } else {
        ...
        if ($course->sec0title) {
            ...
            echo $OUTPUT->heading(get_string('sectionzerodefaultheading', 'format_octabs'), 2, 'fnoutlineheadingblock1', 'sectionzerotextalignment');
        }
    }
    //echo '</td></tr>';
...
if (empty($course->showonlysection0)) {
    ...
    if ($course->numsections > 0) {
        ...
        if (!empty($course->mainheading)) {
            ...
            $strmainheading = get_string('defaultmainheading', 'format_octabs');
        }
        ...
        if (($PAGE->user_is_editing()) || !$completion->is_enabled()) {
            // oncampus echo $OUTPUT->heading($strmainheading, 2, 'fnoutlineheadingblock1');
        } elseif ($completion->is_enabled() && !$isteacher) {
            // oncampus echo $OUTPUT->heading($strmainheading, 2, 'fnoutlineheadingblock ' . $headerextraclass . '');
        } else {
            // oncampus echo $OUTPUT->heading($strmainheading, 2, 'fnoutlineheadingblock1');
        }
